...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        #sorted(w for w in set(word_list) if len(w) >7 and fdist[w] >5)
        #print(sorted(w for w in set(word_list) if len(w) >7 and fdist[w] >5))
        ...
        #print(textList.collocations(settings.SEARCH_TERM))
        ...
        #print(textList.common_contexts(settings.SEARCH_TERM_LIST))

        #Það er hægt að skoða hversu oft tiltekið orð kemur fyrir hlutfallslega:

        print(100*textList.count(settings.SEARCH_TERM) / len(textList))
